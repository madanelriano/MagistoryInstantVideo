...
async function saveAsset(url: string, jobId: string, type: 'image' | 'video' | 'audio'): Promise<string> {
    ...
    const filePath = path.join(path.join((process as any).cwd(), 'temp', jobId), filename);
    ...
    try {
        ...
        for (let i = 0; i < job.segments.length; i++) {
            ...
            await new Promise<void>((resolve, reject) => {
                ...
                if (audioPath) {
                    cmd.input(audioPath);
                    // Standardize segment audio to 44.1k Stereo
                    filters.push(`[${audioInputIndex}:a]aformat=sample_rates=44100:channel_layouts=stereo,volume=1.5[a_norm]`);
                    audioLabel = 'a_norm';
                } else {
                    // Generate silence using filter_complex source (no input file needed)
                    // This fixes 'lavfi not available' on some static builds
                    filters.push(`anullsrc=channel_layout=stereo:sample_rate=44100:duration=${exactDuration}[a_norm]`);
                    audioLabel = 'a_norm';
                }
        ...
        for (const track of job.audioTracks) {
            ...
            // CRITICAL FIX: Force format to 44.1k Stereo BEFORE mixing to avoid silence/dropouts
            complexFilters.push(`[${inputCount}:a]aformat=sample_rates=44100:channel_layouts=stereo,adelay=${delayMs}|${delayMs},volume=${volume}[a${inputCount}]`);
            audioMixInputs.push(`[a${inputCount}]`);
            inputCount++;
        }

        if (audioMixInputs.length > 1) {
            // Use amix with normalize=0 to prevent excessive volume drop, then use dynaudnorm to level it
            complexFilters.push(`${audioMixInputs.join('')}amix=inputs=${audioMixInputs.length}:duration=first:dropout_transition=0:normalize=0,dynaudnorm=p=0.9[aout]`);
